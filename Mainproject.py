    ...
    def __init__(self, parent, controller):
        ...
        def withdraw(amount):
            ...
            if amount>current_balance:
                ...
                mydb=mysql.connector.connect(host="localhost",user="root",password="Kanish@123")
                mycursor=mydb.cursor()
                mycursor.execute("use LICET_ATM")
                mycursor.execute(f"update TABLE1 set balance ={current_balance} where accid = {username1} ")
                mydb.commit()
        ...
        twenty_button=tk.Button(button_frame,text='₹500',font=('orbitron',12),command=lambda:withdraw(20),relief='raised',borderwidth=3,width=30,height=4)
        twenty_button.grid(row=0,column=0,pady=5)

        fourty_button=tk.Button(button_frame,text='₹1000',font=('orbitron',12),command=lambda:withdraw(40),relief='raised',borderwidth=3,width=30,height=4)
        fourty_button.grid(row=1,column=0,pady=5)

        sixty_button=tk.Button(button_frame,text='₹2000',font=('orbitron',12),command=lambda:withdraw(60),relief='raised',borderwidth=3,width=30,height=4)
        sixty_button.grid(row=2,column=0,pady=5)

        eighty_button=tk.Button(button_frame,text='₹2500',font=('orbitron',12),command=lambda:withdraw(80),relief='raised',borderwidth=3,width=30,height=4)
        eighty_button.grid(row=3,column=0,pady=5)

        one_hundred_button=tk.Button(button_frame,text='₹3000',font=('orbitron',12),command=lambda:withdraw(100),relief='raised',borderwidth=3,width=30,height=4)
        one_hundred_button.grid(row=0,column=1,pady=5,padx=794)

        two_hundred_button=tk.Button(button_frame,text='₹4000',font=('orbitron',12),command=lambda:withdraw(200),relief='raised',borderwidth=3,width=30,height=4)
        two_hundred_button.grid(row=1,column=1,pady=5)

        three_hundred_button=tk.Button(button_frame,text='₹5000',font=('orbitron',12),command=lambda:withdraw(300),relief='raised',borderwidth=3,width=30,height=4)
...
class InfoPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        mycursor.execute("use LICET_ATM")
        mycursor.execute(f"select password from TABLE1 where accid = {username1} ")
...
def register_user():
  ...
  mycursor.execute("create database if not exists LICET_ATM")
  mycursor.execute("use LICET_ATM")
  mycursor.execute("create table if not exists TABLE1(accid int(10) primary key,name varchar(30),password char(20),balance char(30))")
  mydb.commit()

  mycursor.execute('select accid from TABLE1')
  ...
  if username_info in b:
    ...
    name_entry.delete(0,END)
        ...
        name_entry.delete(0, END)
        screen1.destroy()
        mycursor.execute("insert into TABLE1 values('"+username_info+"','"+name_info+"','"+password_info+"','"+balance_inti+"')")
        mydb.commit()
      ...
      if username1.isdigit():
          ...
          if int(username1) in user_acc:
              ...
              if password1=='':
                  ...
                  mycursor.execute(f"select name from TABLE1 where accid={username1}")
                  values=mycursor.fetchall()
                  user_name=[]
                  for i in values:
                  ...
                  mycursor.execute(f'select balance from TABLE1 where accid ={username1}')
                  values=mycursor.fetchall()
                  user_balance=[]
                  for i in values:
